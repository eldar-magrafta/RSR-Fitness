...
import { buildHome, showExercises, openModal, closeModal, setOnModalClose, handleOverlayClick, autoSaveExNotes, notePhotoSelected, removeNotePhoto, openNotePhotoViewer, closeNotePhotoViewer, initModalSwipe, deleteExLog, globalExSearchHandler, groupExSearchHandler, openCustomExModal, closeCustomExModal, customExImageSelected, removeCustomExImage, saveCustomEx, deleteCustomEx } from './exercises.js';
...
import { renderNLMeals, nlShowMeal, nlShowPicker, renderNLPicker, nlSearchPicker, nlPickIngredient, nlCloseAmount, nlSetGrams, nlAdjustPickerGrams, nlConfirmAddIng, nlOpenCreateModal, nlCloseCreate, nlCreateMeal, nlIdentifyMealFromPhoto, nlDismissAISkipped, openDeleteMealConfirm, nlToggleFav, nlDuplicateMeal, nlSaveAsSavedMeal, nlUploadMealPhoto, nlRemoveMealPhoto, nlOpenMealPhotoViewer, nlCloseMealPhotoViewer, nlSetSort, nlToggleFavFilter, nlBrowseFoods, nlOpenCustomModal, nlCloseCustom, nlCustomPhotoSelected, nlRemoveCustomPhoto, nlViewCustomPhoto, nlUpdateCustomCal, nlSaveCustom, nlDeleteCustomConfirm, nlAdjustIng, nlRemoveIng, nlSetMealSlot, nlAutoSaveNotes, renderMacroGoals, openMacroGoalsModal, closeMacroGoalsModal, saveMacroGoalsFromModal, initMacroGoalsSwipe, nlSetViewMode, renderNLCalendar, nlPrevMonth, nlNextMonth, nlSelectDate, onMacroCalInput, onMacroPctInput, onMacroPctBlur, setQuickCal, clearDateGoal, resumeDateGoal, openNLFabChoice, closeNLFabChoice, openSavedMealPicker, closeSavedMealPicker, nlFilterSavedMeals, pickSavedMeal, nlOpenRenameModal, nlCloseRename, nlSaveRename, openDeleteAllMealLogs, nlToggleCreateChoice, nlCloseCreateChoice, nlShowBarcodeInput, openMacroWizard, closeMacroWizard, setMacroWizardSex, setMacroWizardActivity, setMacroWizardGoal, updateMacroWizard, applyMacroWizard } from './nutrition.js';
import { nlSearchBarcode, nlCloseBarcodeResult, nlSaveBarcodeAsCustom, nlBarcodePhotoSelected, nlRemoveBarcodePhoto, nlOpenBarcodeScanner, nlCloseBarcodeScanner, nlBarcodeScanFile, initBarcodeSwipe } from './nutrition-barcode.js';
import { openExHistory, setExHistRange, exHistPrevMonth, exHistNextMonth, exHistJumpToDate, renderExHistSets, exHistUpdateSet, exHistUpdateDrop, exHistAddSet, exHistAddDropSet, exHistAddDropStage, exHistRemoveSet, exHistRemoveDrop, openExHistEntry, closeExHistEntry, saveExHistEntry, openDeleteExHistConfirm, initExHistSheetSwipe, openDeleteAllExHist } from './history.js';
...
// Burger-menu overlays whose Back button just returns to the previously active
// tab (all share identical back-handling).
const TAB_OVERLAY_CONTEXTS = new Set(['summary', 'exercise-log', 'prs', 'gallery', 'water', 'muscle-balance']);
...
function handleBack() {
  ...
  if (state.navContext === 'exercise-list') {
    ...
    renderNLMeals();
    renderMacroGoals();
  } else if (TAB_OVERLAY_CONTEXTS.has(state.navContext)) {
    // Full-screen overlays opened from the burger menu — back returns to the
    // tab the user was on. (summary, exercise-log, prs, gallery, water, muscle-balance)
...
window.notePhotoSelected = notePhotoSelected;
window.removeNotePhoto = removeNotePhoto;
window.openNotePhotoViewer = openNotePhotoViewer;
window.closeNotePhotoViewer = closeNotePhotoViewer;
...
window.onMacroPctInput = onMacroPctInput;
window.onMacroPctBlur = onMacroPctBlur;
...
window.openTimelapse = openTimelapse;
window.closeTimelapse = closeTimelapse;
window.generateTimelapse = generateTimelapse;
...
window.exHistUpdateSet = exHistUpdateSet;
window.exHistUpdateDrop = exHistUpdateDrop;
window.exHistAddSet = exHistAddSet;
window.exHistAddDropSet = exHistAddDropSet;
window.exHistAddDropStage = exHistAddDropStage;
window.exHistRemoveSet = exHistRemoveSet;
window.exHistRemoveDrop = exHistRemoveDrop;
...
window.addEventListener('rsr-quota-exceeded', () => {
  showToast('⚠ Local storage full — recent changes may not be saved. Try removing old meal photos.', {
    background: 'linear-gradient(135deg, #ff3d71, #c93764)',
    duration: 5000,
  });
});
